...
using ui.UIWindowColor.Infrastructure.Models.@interface;
using UnityEngine;

namespace ui.UIWindowColor.Infrastructure.Models
{
    public class ModelCardForClassic : IModelCard
    {
        private readonly List<CardData> _cards;
        public event Action<IModelCard> OnCardClicked;

        public ModelCardForClassic(StateCard state, List<CardData> cards, int numberCard, Sprite closeCardImage, Sprite openCardImage, Sprite attentionCardImage)
        {
            SetIDCard(cards);
            State = state;
            _cards = cards;
        ...
        public string IDCard { private set; get; }
